...
function getToken(token, id, secret, url) {
    ...
    .catch( () => {
        ...
        .then( (res) => {
            // Check if what we got back is valid JSON. Do this by converting to string and then back to object.
            try {
                return JSON.parse(JSON.stringify(res));
            } 
            catch (err) {
                misc.debug(whoami + 'Throwing InvalidJSONError');
                throw new misc.error('InvalidJSONError', 'The HTTP request was successful but returned invalid data');
            }
        })
        .catch( (err) => {
            throw err;
