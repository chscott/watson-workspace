...
function getToken(token, id, secret, url) {
    ...
    .catch( () => {
        ...
        misc.trace.logTrace(whoami + 'Requesting a new token from ' + tokenUrl, misc.trace.TRACE_LEVEL_MEDIUM);
        ...
        .catch( (err) => {
            misc.trace.logTrace(whoami + 'Throwing request error', misc.trace.LOG_LEVEL_LOW);
            throw err;
        });
...
function getTokenPromise(token) {
    ...
    return new Promise( (resolve, reject) => {
        ...
        if (!token || !token.access_token) {
            misc.trace.logTrace(whoami + 'Invalid token', misc.trace.TRACE_LEVEL_MEDIUM);
            return reject(new Error('InvalidTokenError'));
        // If the token is valid, resolve Promise
        } else {
            misc.trace.logTrace(whoami + 'Token is valid', misc.trace.TRACE_LEVEL_MEDIUM);
...
function verifyWebhook(req, res, secret) {
    ...
    misc.trace.logTrace(whoami + 'Received a verification request from Watson Workspace', misc.trace.TRACE_LEVEL_MEDIUM);
    ...
    .catch( (err) => {
        misc.trace.logTrace(whoami + 'Throwing WebhookVerificationError', misc.trace.LOG_LEVEL_LOW);
