...
function getUserById(id, token) {
    ...
    .then( (res) => inspectGraphQLResponse(res, id) )
    .catch( (err) => {
        throw err;
    });
...
function getUserByEmail(email, token) {
    ...
    query.variables = { "userEmail": email };
    misc.debug(whoami + 'GraphQL query with variables: ' + JSON.stringify(query));
    ...
    return Promise.resolve()
    .then( () => request(options) )
    .then( (res) => inspectGraphQLResponse(res, email) )
    .catch( (err) => {
        throw err;
    });
...
function inspectGraphQLResponse(res, identifier) {
    ...
    misc.debug(whoami + 'User data: ' + JSON.stringify(res.data)); 
    return res.data; 
